...
\subsection{Results}
Figure~\ref{fig:lsa} and Figure~\ref{fig:lda} visualize the results we obtained for different dimensionalities of LSA, LDA and the failed HDP attempt, together with the baseline performance. \\
\begin{figure}
...
\begin{figure}
  ...
  \label{fig:lda}
\end{figure}
As you can see, LSA outperforms LDA, and achieves its highest result of 0.641 at 350 topics with $k=10$. The best performance for LDA was a recall of 62\% at 150 topics. The HDP model only achieved a recall of 47.4\%, which is actually below baseline performance (which is possible as only values of k below 50 were considered). Other systems in SensEval-3 ranged from approximately 60\% to 73\% recall\cite{senseval3paper}, so while our system is not on par with the state of the art in its current form, it performs reasonably well and has potential to be improved. 
